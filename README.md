...
To build the development version of gate-core, clone this repository, then run `mvn install` in the top-level directory.  To run the GATE Developer GUI, you also need to run `mvn compile` in the `distro` directory, after which you can run the GUI from the `distro` directory as appropriate to your platform (`bin/gate.sh` on Linux, open `GATE.app` on Mac, or run `gate.exe` on Windows). When you update, it's the same procedure.
